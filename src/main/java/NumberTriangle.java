...
public class NumberTriangle {
    ...
    public int retrieve(String path) {
        ...
        String nextGo = path.substring(1);
        if (firstChar == 'l') {
            NumberTriangle triangle = this.left;
            return triangle.retrieve(nextGo);
        } else {
            NumberTriangle triangle = this.right;
            return triangle.retrieve(nextGo);
        }
    }
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while (line != null) {
            ...
            int i = 0;
            for (NumberTriangle triangle : prev) {
                NumberTriangle newleft = new NumberTriangle(Integer.parseInt(numbers[i]));
                NumberTriangle newright = new NumberTriangle(Integer.parseInt(numbers[i + 1]));
                triangle.setLeft(newleft);
                triangle.setRight(newright);
                prev.set(i, newleft);
                i++;
            }
            prev.add(new NumberTriangle(Integer.parseInt(numbers[i])));
